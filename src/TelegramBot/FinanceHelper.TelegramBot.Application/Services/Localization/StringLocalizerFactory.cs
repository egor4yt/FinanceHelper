...
internal class StringLocalizerFactory : IStringLocalizerFactory
{
    ...
    private readonly ConcurrentDictionary<(string, string), IStringLocalizer> _localizerCache = new ConcurrentDictionary<(string, string), IStringLocalizer>();

    public IStringLocalizer Create(Type objectToLocalizationType, string culture)
    {
        if (_localizerCache.TryGetValue((objectToLocalizationType.AssemblyQualifiedName!, culture), out var localizer) == false)
        {
            var typeInfo = objectToLocalizationType.GetTypeInfo();
            var baseName = GetLocalizationFilePrefix(typeInfo);

            localizer = CreateLocalizationManagerStringLocalizer(baseName, culture);

            _localizerCache.TryAdd((objectToLocalizationType.AssemblyQualifiedName!, culture), localizer);
        }
    ...
    private static LocalizationManagerStringLocalizer CreateLocalizationManagerStringLocalizer(string baseName, string culture)
    {
        var localizationManager = new LocalizationManager(baseName);
        return new LocalizationManagerStringLocalizer(localizationManager, culture);
    }
