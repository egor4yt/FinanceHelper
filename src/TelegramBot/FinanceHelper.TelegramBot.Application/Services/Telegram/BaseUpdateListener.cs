...
internal class BaseUpdateListener(ITelegramBotClient botClient, IStringLocalizerFactory stringLocalizerFactory, IMessageBroker messageBroker) : IUpdatesListener
{
    /// <summary>
    ...
    [
        new StartCommand(botClient, messageBroker),
        new AdminCommand(botClient, messageBroker)
    ];
    ...
    public async Task SendUpdateAsync(Update update)
    {
        ...
        var command = _commands.FirstOrDefault(x => x.Command == message.Text) ?? new UnknownCommand(botClient, messageBroker);
